...
| `LAW_MCP_ERROR_MESSAGE_MAX_CHARS` | `500` | Cap on error-message length for categories whose message text is built from an exception's own string (`validation`, `not_found`, `precondition`, `content_too_large`, `unavailable`), which can be of unbounded length; accepted range `80`-`10000`. Truncation is announced in the message rather than silent; a message that ends in a URL (the source PDF of an oversized act) keeps that URL whole when the rest of the text can absorb the cut, otherwise the plain cut applies so the cap always holds |
